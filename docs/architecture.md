...
Past `FM_WEDGE_FORCE_ACTION_COUNT` escalations on the same unchanged pane, the watcher stops adding more instruction text and mechanically changes the wake reason's leading verb from `stale:` to `blocked:` instead, since a supervising agent's compliance with instruction text alone degrades over a long session.
...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, PR-discovery fallback, stale-lock recovery procedure, and the slot-identity ownership check that refuses unconditionally (including under `--force`) when a recorded worktree no longer belongs to the task.
