...
test_auto_pick_compares_reset_instants_not_strings() {
  ...
  fakebin=$(fm_fakebin "$home")
  fm_write_meta "$home/state/t13.meta" "worktree=$home"
...
SH
  chmod +x "$fakebin/fake-quota-axi"
  FM_HOME="$home" FM_QUOTA_AXI_BIN="$fakebin/fake-quota-axi" "$TIMER" t13 >/dev/null \
    || fail "auto-pick arm should succeed"
  assert_grep "RESET_EPOCH=1906516800" "$home/state/t13.check.sh" \
...
test_a_ragged_reset_cannot_corrupt_the_generated_check() {
  ...
  fm_write_meta "$home/state/t14.meta" "worktree=$home"
...
test_refuses_a_reset_with_an_interior_newline() {
  ...
  fm_write_meta "$home/state/t16.meta" "worktree=$home"
  status=0
  FM_HOME="$home" "$TIMER" t16 --reset "$(printf '2099-01-01\nT00:00:00+00:00')" \
    >/dev/null 2>"$home/err.txt" || status=$?
  ...
  assert_absent "$home/state/t16.check.sh" "a reset with an interior newline armed a check anyway"
  assert_absent "$home/state/t16.check-trust" "a reset with an interior newline registered a trust record"
...
test_generated_check_is_syntax_checked_before_install() {
  ...
  fm_write_meta "$home/state/t15.meta" "worktree=$home"
  FM_HOME="$home" "$TIMER" t15 --reset "2099-01-01T00:00:00+00:00" >/dev/null \
    || fail "arming with a future reset should succeed"
  bash -n "$home/state/t15.check.sh" \
