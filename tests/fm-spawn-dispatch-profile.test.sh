...
# hence which billing account) must cascade to the launched claude agent. A
# cwd-scoped selector never fires inside the disposable worktree, so without the
# per-launch prefix the agent silently reverts to ~/.claude and can bill the wrong
# account.
test_claude_cascades_spawner_config_dir() {
  local rec id out status launch
  id=profile-cfgdir-z1a
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "CLAUDE_CONFIG_DIR='/tmp/acct with space' CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude " \
    "claude launch did not cascade the spawner's CLAUDE_CONFIG_DIR (space-safe) as a per-launch prefix"
  pass "claude launch cascades the spawner's CLAUDE_CONFIG_DIR verbatim (space-safe)"
}

# With no config-dir convention in effect (CLAUDE_CONFIG_DIR unset), the prefix must
# fall back to claude's own documented default so an unrelated home is unaffected.
test_claude_config_dir_defaults_when_unset() {
  local rec id out status launch fakehome
  id=profile-cfgdir-default-z1b
  rec=$(make_spawn_case profile-cfgdir-default claude "$id")
  read_case_record "$rec"
  ...
  out=$( unset CLAUDE_CONFIG_DIR ANTHROPIC_BASE_URL ANTHROPIC_API_KEY
    ...
    run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" )
  status=$?
  expect_code 0 "$status" "claude spawn should succeed with no scoped config dir"
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "CLAUDE_CONFIG_DIR='$fakehome/.claude' CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude " \
    "claude launch did not default CLAUDE_CONFIG_DIR to \$HOME/.claude when unset"
